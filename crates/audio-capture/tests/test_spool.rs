...
fn test_atomic_spool_write_and_verification() {
    ...
    // Seal manifest
    spool
        .seal_manifest("test-inv", TrackType::Interviewer, 0, 2, 2000)
        .unwrap();

    // Verify spool
    let report = spool
        .verify_track_spool("test-inv", TrackType::Interviewer)
        .unwrap();
...
fn test_corrupted_chunk_detected() {
    ...
    let chunk_path = spool
        .get_track_dir("test-inv", TrackType::Candidate)
        .join("00000000.chunk");
    fs::write(&chunk_path, b"CORRUPTED_BYTES").unwrap();

    let report = spool
        .verify_track_spool("test-inv", TrackType::Candidate)
        .unwrap();
    assert!(!report.is_valid);
